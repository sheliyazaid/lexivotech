        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-px bg-ink/10 border border-ink/10 rounded-2xl overflow-hidden">
          {SERVICES.map((s, i) => (
            <motion.article
              ...
              data-cursor="hover"
            >
              <div className="absolute inset-0 bg-ink translate-y-full group-hover:translate-y-0 transition-transform duration-700 ease-[cubic-bezier(0.7,0,0.2,1)]" />
              <div className="relative z-10 flex items-start justify-between">
                <span className="font-body uppercase tracking-[0.3em] text-[10px] text-ink/50 group-hover:text-bone/60 transition-colors">{s.n}</span>
                <ArrowUpRight size={18} className="text-ink/40 group-hover:text-bone group-hover:-translate-y-1 group-hover:translate-x-1 transition-all duration-500" />
              </div>
              <div className="relative z-10">
                <h3 className="font-brand uppercase text-2xl md:text-3xl leading-tight text-ink group-hover:text-bone transition-colors duration-500">
                  {s.title}
                </h3>
                <p className="mt-4 font-body text-sm text-ink/65 group-hover:text-bone/75 transition-colors duration-500 max-h-0 group-hover:max-h-40 overflow-hidden opacity-0 group-hover:opacity-100 transition-[max-height,opacity] ease-out">
                  {s.desc}
                </p>
              </div>
            </motion.article>
